...
You can use organizational units \(OUs\) to group accounts together to administer as a single unit\. This greatly simplifies the management of your accounts\. For example, you can attach a policy\-based control to an OU, and all accounts within the OU automatically inherit the policy\. You can create multiple OUs within a single organization, and you can create OUs within other OUs\. Each OU can contain multiple accounts, and you can move accounts from one OU to another\. However, OUs must be unique within a root \(no duplications of names\)\.
...
1. If the tree view pane is not visible on the left side of the page, choose the **TREE VIEW** switch icon ![\[Image NOT FOUND\]](http://alpha-docs-aws.amazon.com/organizations/latest/userguide/images/switch-icon.png)\.
...
1. Choose the OU or root that you want to navigate to\. The node in the tree view that is displayed in boldface text is the one that you are currently viewing in the center pane\.
...
1. \(Optional\) If you want to create an OU inside an existing OU, navigate to the child OU by choosing the name \(not the check box\) of the child OU, or by choosing the OU in the tree view\.
...
You can use the following commands to create an OU:

+ AWS CLI: [aws organizations create\-organizational\-unit](http://alpha-docs-aws.amazon.com/cli/latest/reference/organizations/create-organizational-unit.html)

+ AWS API: [CreateOrganizationalUnit](http://alpha-docs-aws.amazon.com/organizations/latest/APIReference/API_CreateOrganizationalUnit.html)
...
1. On the **Organize accounts** tab, navigate to the parent of the OU that you want to rename\. Select the check box for the child OU that you want to rename\.
...
You can use the following commands to rename an OU:

+ AWS CLI: [aws organizations update\-organizational\-unit](http://alpha-docs-aws.amazon.com/cli/latest/reference/organizations/update-organizational-unit.html)

+ API: [UpdateOrganizationalUnit](http://alpha-docs-aws.amazon.com/organizations/latest/APIReference/API_UpdateOrganizationalUnit.html)

## Moving an Account to an OU or Between the Root and OUs<a name="move_account_to_ou"></a>

When signed in to your organization's master account, you can move accounts in your organization from the root to an OU, from one OU to another, or back to the root from an OU\. Placing an account inside an OU makes it subject to any policies that are attached to the parent OU and any other OUs in the parent chain up to the root\. If an account is not in an OU, then it is subject to only the policies that are attached to the root and any that are attached directly to the account\. To move an account, complete the following steps\.
...
1. Choose the **Organize accounts** tab, and then navigate to the OU that contains the account that you want to move\. When you find the account, select its check box\. Select multiple check boxes if you want to move multiple accounts\.
...
You can use the following commands to move an account:

+ AWS CLI: [aws organizations move\-account](http://alpha-docs-aws.amazon.com/cli/latest/reference/organizations/move-account.html)

+ API: [MoveAccount](http://alpha-docs-aws.amazon.com/organizations/latest/APIReference/API_MoveAccount.html)
...
1. On the **Organize accounts** tab, navigate to the parent container of the OU that you want to delete\. Select the OU's check box\. You can select check boxes for multiple OUs if you want to delete more than one\.
...
You can use the following commands to delete an OU: 

+ AWS CLI: [aws organizations delete\-organizational\-unit](http://alpha-docs-aws.amazon.com/cli/latest/reference/organizations/delete-organizational-unit.html)

+ API: [DeleteOrganizationalUnit](http://alpha-docs-aws.amazon.com/organizations/latest/APIReference/API_DeleteOrganizationalUnit.html)