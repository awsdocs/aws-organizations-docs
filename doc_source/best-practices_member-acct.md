...
+ Unlike our [recommendation for the management account root user](orgs_best-practices_mgmt-acct.md#best-practices_mgmt-acct_mfa), for member accounts you can re\-use that physical MFA device for multiple member accounts\. However, be aware that reusing this MFA device can create issues when teams are geographically distributed\.

  If you choose to use a virtual MFA application instead of a dedicated hardware device, you can address this geographic limitation by printing and securely storing the QR code used to configure the account in the application\. Document the QR code's purpose, and seal and store it in accessible safes across the time zones you operate in, according to your information security policy\. 
