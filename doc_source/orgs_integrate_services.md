...
**Important**  
We recommend that you enable trusted access by using the trusted service's console, or the AWS CLI or API operations in one of the [AWS SDKs](https://aws.amazon.com/tools/)\. This enables the trusted service to perform any required initialization or create any required resources\. To learn what configuration the trusted service performs, see the documentation for that service\. 
