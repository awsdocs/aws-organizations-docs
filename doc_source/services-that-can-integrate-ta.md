...
**Important**  
We recommend that where possible, you use the AWS Trusted Advisor console or tools to enable integration with Organizations so that AWS Trusted Advisor can perform any configuration that it requires\. Proceed with these steps only if you can’t enable integration using the tools provided by AWS Trusted Advisor\.  
If you enable trusted access by using the tools provided by AWS Trusted Advisor then you don’t need to complete these steps\.
...
1. In the **Integrated services** section, find the row for **AWS Trusted Advisor**, choose the service’s name, and then choose Enable trusted access\.

1. In the confirmation dialog box, enable **Show the option to enable trusted access**, enter **enable** in the box, and then choose **Enable trusted access**\.

1. If you are the administrator of only AWS Organizations, tell the administrator of AWS Trusted Advisor that they can now enable that service using its console to work with AWS Organizations\.
...
**Important**  
We recommend that where possible, you use the AWS Trusted Advisor console or tools to disable integration with Organizations so that AWS Trusted Advisor can perform any cleanup steps that it requires\. Proceed with these steps only if you can’t disable integration using the other service’s tools\.  
If you are the administrator of only AWS Organizations and not AWS Trusted Advisor, wait until the administrator of AWS Trusted Advisor tells you that they disabled integration with that service’s console or tools, and that any resources have been cleaned up\.
...
1. In the **Integrated services** section, find the row for **AWS Trusted Advisor** and then choose the service’s name\.
